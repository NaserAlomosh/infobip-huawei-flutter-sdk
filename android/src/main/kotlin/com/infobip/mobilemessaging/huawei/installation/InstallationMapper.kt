...
internal object InstallationMapper {
    ...
    fun toMap(value: Installation): Map<String, Any?> =
        mapOf(
            ...
            ChannelContract.CUSTOM_ATTRIBUTES to
                    UserMapper.channelValue(value.customAttributes),
        )
        ...
        val map =
            payload as? Map<*, *>
                ?: throw IllegalArgumentException(
                    "installation must be a map",
                )

        boolean(
            map,
            ChannelContract.IS_PRIMARY_DEVICE,
        )?.let {
            target.isPrimaryDevice = it
        }

        if (map.containsKey(ChannelContract.CUSTOM_ATTRIBUTES)) {
            target.customAttributes =
                UserMapper.toNativeCustomAttributes(
